//LIBRARY AMENDED FOR USE BY PUC DASHBOARD V3:
// - added callback from printMap cascading to createImagePlaceholder, resizeAndPrintMap, pausePrint, and printOpertion
...
L.Control.EasyPrint = L.Control.extend({
  ...
  printMap: function (event, filename, callback) {
    ...
    if (sizeMode === 'CurrentSize') {
      return this._printOpertion(sizeMode, callback);
    }
    ...
    this._createImagePlaceholder(sizeMode, callback) 
  ...
  _createImagePlaceholder: function (sizeMode, callback) {
    ...
    domtoimage.toPng(this.mapContainer, {
      ...
      .then(function (dataUrl) {
        ...
        plugin._resizeAndPrintMap(sizeMode, callback);
      })
  ...
  _resizeAndPrintMap: function (sizeMode, callback) {
    ...
    if (this.options.tileLayer) {
      ...
      this._printOpertion(sizeMode, callback)
    }
  },

  _pausePrint: function (sizeMode, callback) {
    var plugin = this
    var loadingTest = setInterval(function () { 
      if(!plugin.options.tileLayer.isLoading()) {
        clearInterval(loadingTest);
        plugin._printOpertion(sizeMode, callback)
      }
    }, plugin.options.tileWait);
  },

  _printOpertion: function (sizemode, callback) {
    var plugin = this;
  ...
  _a4PageSize: {
    width: 600, //660, //700, //1045,
    height: 400  //440, //500  //715
